<script lang="ts">
	...
	let {
		...
		siteUrl: string;
		rssSourceUrl: string;
		// When set, the feed URL is assembled elsewhere (RSS-Bridge params, page-feed config) and
		// shown read-only here; null means the user edits it directly.
		feedUrlManagedBy: 'rss-bridge' | 'page-feed' | null;
			...
			{#if feedUrlManagedBy}
				...
				<p class="mt-1 text-xs text-n-500">
					{#if feedUrlManagedBy === 'page-feed'}
						Generated by MicroRSS from a listing page — edit the page and selectors in the Page Feed tab.
					{:else}
