<script lang="ts">
	...
	// svelte-ignore state_referenced_locally
	const action0 = loadFilterAction(feed.id);
	const usingClient0 = action0 === 'mark-read';
	// svelte-ignore state_referenced_locally
	const hideRules0 = loadHideRules(feed.id);
	// svelte-ignore state_referenced_locally
	...
	// svelte-ignore state_referenced_locally
	let initialFilterAction = $state(action0);
	let initialHideSig = $state(
		JSON.stringify(
			usingClient0
	...
	async function persistChanges(changes: FeedUpdate) {
		...
		// Client-side filter action + hide rules (localStorage). In block mode the rules live in
		// Miniflux, so the client hide list is cleared.
		storageSet(FILTER_ACTION_PREFIX + feed.id, filterAction);
		saveHideRules(feed.id, effHideRules);
		initialFilterAction = filterAction;
		initialHideSig = effHideSig;
